// LOKASI: src/pages/Cashier/Cashier.jsx (FINAL DENGAN FITUR PELANGGAN)
...
import CustomerModal from '../../components/CustomerModal'; // <-- IMPORT BARU
import './Cashier.scss';

function CashierPage() {
  const navigate = useNavigate();
  // ... (semua state lama tetap sama)
  ...
  const [isCustomerModalOpen, setIsCustomerModalOpen] = useState(false); // <-- STATE BARU
  const [lastTransaction, setLastTransaction] = useState(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('Semua');
  const [discount, setDiscount] = useState(0);

  // --- STATE BARU UNTUK PELANGGAN ---
  const [selectedCustomer, setSelectedCustomer] = useState(null);

  // ... (semua fungsi lama hingga handleApplyDiscount tetap sama) ...
  const loadMenus = async () => { /* ... */ };
  useEffect(() => { loadMenus(); }, []);
  const uniqueCategories = useMemo(() => { /* ... */ }, [menus]);
  const filteredMenus = useMemo(() => { /* ... */ }, [menus, searchTerm, selectedCategory]);
  const handleLogout = () => { /* ... */ };
  const resetDiscount = () => { /* ... */ };
  const addToCart = (menu) => { /* ... */ };
  const updateQuantity = (menu_id, amount) => { /* ... */ };
  const removeFromCart = (menu_id) => { /* ... */ };
  const clearCart = () => { setCart([]); setDiscount(0); setSelectedCustomer(null); }; // <-- Tambah reset customer
  ...
  const handleProcessTransaction = async (paymentMethod, cashTendered, change) => {
    ...
    const transactionData = {
      ...
      change: change,
      discount: discount,
      customer_id: selectedCustomer ? selectedCustomer.ID : null, // <-- KIRIM CUSTOMER ID
    };
    try {
  ...
  return (
    <div className="cashier-layout">
      {/* ... (Header dan Menu Section tidak berubah) ... */}
      <aside className="cart-section">
        ...
        <div className="customer-display">
          ...
          <button onClick={() => setIsCustomerModalOpen(true)} className="change-customer-button">
        ...
        {/* ... (Cart Items dan Cart Summary tidak berubah) ... */}
      </aside>
...
// KODE LENGKAP UNTUK ANDA COPY-PASTE
function FullCashierPage() {
    // ... (Salin semua state dan fungsi dari atas)
    const navigate = useNavigate();
    const [menus, setMenus] = useState([]);
    const [cart, setCart] = useState([]);
    const [loading, setLoading] = useState(true);
...
export default FullCashierPage;